...
(function() {
  ...
  countCode = function(password, key) {
    var code1, code16, code32, i, md5one, md5three, md5two, rule, source, str, length;
    if (!password || !key) {
      return;
    }
    if (version === '2') {
    ...
    if (isNaN(code1)) {
      code16 = code32.slice(0, length);
    } else {
      code16 = "L" + code32.slice(1, length);
    }
    if (version === '2') {
      code16 = code16.split("");
      i = 0;
      ...
      while (i < code16.length) {
        ...
        if (code16[i].match(/[0-9]/) && !flags.num) {
          code16[i] = '#';
          flags.num = true;
        }
        if (flags.lower && flags.num) {
          break
        }
        i++;
      }
  ...
  $('[name="version"]').on("change", function(ev) {
    version = ev.target.value;
    result.val(countCode(input_pswd.val(), input_ident.val()));
  });
